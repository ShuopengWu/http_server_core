...
ISocket::ISocket()
{
    ...
    util::error_assert(socket_fd == -1, "Create socket failed");
    set_socket_reusaddr();
...
void ISocket::listen(int backlog)
{
    ...
    util::error_assert(result == -1, "Socket listen failed");
...
void ISocket::set_nonblocking()
{
    ...
    util::error_assert(result == -1, "Failed to set socket to non-blocking state");
